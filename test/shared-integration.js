  ...
  return new Promise((resolve, _reject) => {
    const artilleryProcess = spawn(
      ...
      [
        ...
        '--output',
        `shared-test-${configName.replace('.yml', '')}-results.json`,
      ],
      {
        stdio: 'inherit',
        cwd: process.cwd(),
    ...
    artilleryProcess.on('close', (code) => {
      if (code === 0) {
        ...
        console.log(
          `⚠️  Shared test ${configName} completed with code ${code} (some failures expected without real S3 data)`
        )
...
async function runConnectivityTest() {
  ...
  console.log(
    `📊 Connectivity Results: ${passedTests}/${testCases.length} tests handled gracefully`
  )
}
...
async function main() {
  try {
    ...
    console.log(
      '🔧 To run with real S3 data, configure AWS credentials and use bucket: s3proxy-public'
    )
  } catch (error) {
